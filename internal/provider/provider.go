		...
		Attributes: map[string]schema.Attribute{
			...
			"destroy_protection": schema.BoolAttribute{
				Description: "When true, the provider refuses DELETE requests at the " +
					...
					"flag is cleared. Equivalent to per-resource `deletion_protection` " +
					"flags found in major Terraform providers, but enforced at the wire " +
					"for every resource in the provider at once. Layer it with " +
					"`read_only=true` for Phase 0 " +

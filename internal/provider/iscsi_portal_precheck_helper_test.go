...
// has an iSCSI portal bound to 0.0.0.0. Middleware permits exactly one,
// so a pre-existing wildcard portal (a CSI driver's, or litter from an
// aborted run) turns every portal test into an identical apply failure.
// That is an environment collision, not a provider defect, so it reads
// as a skip naming the offending portal.
//
// Duplicated from the resources package on purpose: the two test
// packages share no helper file, and a test-only helper is not worth an
// exported surface.
//
// Deliberately NOT named acc_*_test.go: TestIdempotencyCheckCoverage
// scans that pattern for acceptance tests carrying a
// PostApplyPostRefresh ExpectEmptyPlan check, and a helper with no test
// steps would trip it.

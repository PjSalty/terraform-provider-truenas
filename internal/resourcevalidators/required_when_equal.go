...
// ConfigValidators — the Plugin Framework equivalent of the AWS
// provider's `ConflictsWith`, `ExactlyOneOf`, and conditional
// requirement helpers. Each validator returned by this package is
...
//
// Typical use is the AWS `create_type`/`protocol`/`auth_method`
// discriminator pattern, where one enum value gates which other
// attributes are meaningful. For example, truenas_certificate uses

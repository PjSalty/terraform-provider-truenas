...
- **Code comments + CHANGELOG** — scrubbed all "AWS-grade",
  "AWS-quality", "AWS-parity", "AWS-style", "AWS-PARITY MILESTONE",
  "AWS provider pattern", and "AWS-provider-style" phrasing. Each
  became a neutral phrase describing the thing itself
  ("battle-hardened" for invariants, "standard" for patterns,
  "destructive resources" for the provider pattern reference).
  The provider stands on its own merits; comparison framing added
  noise.
...
- **`examples/prod-smoke/`** — a committed, version-controlled copy
  ...
  - `versions.tf` — provider pin matching the `~/.terraformrc`
    dev_override (source `saltstice/truenas`, binary staged at
...
- **Every package at 100.0% statement coverage.** The CI pipeline's
  per-package 100% coverage gate (`.gitlab-ci.yml:65`) now passes
...
- **`internal/resources/cloudsync_credential_test.go`** —
  ...
  modifiers end-to-end without requiring live TrueNAS or AWS
  credentials.
...
- **`internal/client/redact.go`** — every non-2xx response body is
  ...
  `auth`, `credential`, `passphrase`, AWS STS token names, and
  more. Non-JSON error bodies are truncated at 512 bytes with a
...
### Phase E — config-time cross-attribute validators (AWS ConfigValidators parity)
...
- **`client.DestroyProtection` + `ErrDestroyProtected`**: a second
  ...
  the wire. Matches AWS provider's `deletion_protection` pattern
  (aws_db_instance, aws_eks_cluster, aws_lb) except enforced for
...
### Added — 1-to-1 AWS provider parity release
...
- **Community infrastructure**: SECURITY.md, CODE_OF_CONDUCT.md,
  .gitlab/CODEOWNERS, MR/issue templates, pre-commit hooks, renovate,
  .changelog/ + changie, markdownlint, yamllint.
- **Expanded CONTRIBUTING.md** (~200 lines) with AWS-aligned workflow,
  resource addition checklist (incl. skaff), quality gates table.
...
- **Hand-written docs + examples for all 62 resources.** `docs/resources/*.md`
  follows the AWS provider format (frontmatter + intro + Example Usage +
  Argument Reference + Attribute Reference + Timeouts + Import). Each attribute
...
- **Acceptance test suite expanded from 30 → 156 functions across 58 files**
  ...
  recover from external drift — the same pattern used by
  hashicorp/terraform-provider-aws. All acceptance tests still gated on
  `TF_ACC=1` and run against the test VM only.
- **Test suite expanded from 221 → 1079 passing unit tests** (all automated via
  GitLab CI `go test -race -coverprofile`). Adds comprehensive httptest-mocked
...
- Repo polish: `.golangci.yml` (13 linters, zero findings), `Makefile`
  with standard HashiCorp targets, `.editorconfig`, and new GitLab CI

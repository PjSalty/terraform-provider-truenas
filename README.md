...
Unlike per-resource `deletion_protection` flags found in many providers,
this is enforced at the wire for every resource at once, so there is no
per-resource coverage gap to audit.
...
Every GitHub release is signed with a GPG key whose public counterpart is
committed at [`docs/gpg-public-key.asc`](docs/gpg-public-key.asc). The
signing fingerprint is `29A6 D319 E411 670F 561E  2B9C EC8F 6B9D 7DB7 49E7`.
The Terraform Registry verifies the signature automatically on
`terraform init`. For manual verification steps, see

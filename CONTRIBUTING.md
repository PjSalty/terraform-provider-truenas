...
conventions and the quality bar established by
[hashicorp/terraform-provider-aws](https://github.com/hashicorp/terraform-provider-aws).
If you've contributed to terraform-provider-aws, you will find the layout
familiar.
...
4. **Register** the resource in `internal/provider/provider.go` under
   `Resources()`.
5. **Add matching data source** in `internal/datasources/<name>.go` — AWS
   provider convention is every resource has a matching data source.
6. **Add tests**:
...
Commit the resulting file as part of your MR. At release time, a maintainer
